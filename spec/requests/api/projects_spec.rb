...
describe API::Projects do
  ...
  describe 'GET /users/:user_id/projects/' do
    ...
    context "when user's username is used" do
      it 'returns projects filtered by user' do
        get api("/users/#{URI::encode(user4.username)}/projects/", user)
  ...
  describe 'POST /projects/user/:id' do
    ...
    context "when user's username is used" do
      it 'creates new project without path but with name and return 201' do
        expect { post api("/projects/user/#{URI::encode(user.username)}", admin), name: 'Foo Project' }.to change {Project.count}.by(1)
        expect(response).to have_gitlab_http_status(201)
